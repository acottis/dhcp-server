	...
	loop {
		...
		if let Some(dhcp) = dhcp {
			...
			match dhcp.msg_type {
				...
				_ => {
					log::info!(
						"Unable to handle, not implemented: {:?}",
						dhcp.msg_type
					);
					continue;
				}
...
impl<'a> DHCP<'a> {
	/// Read an incoming packet and parse out DHCP information if it is a DHCP
	/// packet
	fn parse(payload: &'a [u8; BUFFER_SIZE], packet_len: usize) -> Option<Self> {
		...
		loop {
			...
			if payload[options_ptr] == 255 {
				break;
			}

			// Not enough space to have length in the option
			if options_ptr + 1 > packet_len {
				break;
			}
			...
			let res: Option<Options> = match &payload[options_ptr] {
				...
				94 => {
					...
					pxe_config.version = (data[0], data[1], data[2]);
					None
				}
				// Client Identifier (For PXE)
				97 => {
					...
					pxe_config
						.client_id
						.copy_from_slice(&data[..data.len() - 1]);
					None
				}
				opt => {
